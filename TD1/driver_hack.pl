...
#
if ($USE_BTEST) {
    print "\n4. Compiling and running './btest -g ' to determine performance score.\n";
    system("cp Zap-bits.c bits.c");
    ...
    print "\n4. Running './bddcheck/check.pl -g ' to determine performance score.\n";
    system("cp Zap-bits.c bits.c");
    $status = system("./bddcheck/check.pl -g > btest-Zapped.out 2>&1");
    if ($status != 0) {
...
#
open(INFILE, "/home/achille/dlc-opcount-hack.out") 
